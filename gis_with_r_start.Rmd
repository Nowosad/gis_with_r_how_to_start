...
<iframe src="05_leaflet.html" height="600" width="700"></iframe> 
...
world_globe <- sf::st_transform(world,
        "+proj=ortho +x_0=0 +y_0=0 +lon_0=-100 +lat_0=40 +ellps=WGS84 +no_defs")
...
my_places = c("New York", "Los Angeles", "Chicago", "Houston", "Philadelphia")

output <- my_places %>%
        map(opencage_forward, limit = 1) %>% 
        map_df(1)

output_sf <- st_as_sf(output, coords = c("geometry.lng", "geometry.lat"), crs = 4326)
plot(output_sf$geometry, axes = TRUE)

m1 = mapview(output_sf)
saveWidget(m1, file = "08_geocoding.html", selfcontained = FALSE)
```
